...
export interface Diagnostic {
  line: number; // 1-based
  column: number; // 0-based, inclusive start
  endColumn: number; // 0-based, exclusive end
  severity: Severity;
  message: string;
}
...
export function lintSource(dataset: Dataset, source: string): Diagnostic[] {
  const diagnostics: Diagnostic[] = [];
  const lines = source.split(/\r?\n/);
  for (let i = 0; i < lines.length; i++) {
    const code = stripComment(lines[i] as string);
    for (const match of code.matchAll(LOVE_CHAIN)) {
      const path = match[0];
      if (isKnownApi(dataset, path)) continue;
...
function isKnownApi(dataset: Dataset, path: string): boolean {
  if (dataset.lookup(path)) return true;
  const prefix = path + ".";
